...
st.markdown(
    ...
    <div class="navbar">
        ...
        <h2 style="color:#F0B90B;">USDT Spot-Trading Dashboard (CoinCap)</h2>
    </div>
    ''',
    unsafe_allow_html=True
)

st.markdown("### 🔄 CoinCap-Powered Real-Time Insights + LLM Predictions")
...
def fetch_coincap_data():
    ...
    try:
        ...
        st.error(f"CoinCap API Error: {str(e)}")
        return pd.DataFrame()
...
def get_llm_prediction(name, confidence, risk, rsi):
    ...
    return f"LLM Insight: Based on {name}'s RSI ({rsi:.1f}) and price action, the market looks **{trend}**. Movement expected in {int((100 - confidence) * 0.2)} to {int((100 - confidence) * 0.5)} minutes."
...
if st.button("🔍 Run Analysis"):
    with st.spinner("Fetching market data from CoinCap (cached)..."):
        df = fetch_coincap_data()
        if not df.empty:
            for idx, row in df.iterrows():
                name = row['name']
                symbol = row['symbol'].upper()
                price = float(row['priceUsd'])
                logo_url = f"https://assets.coincap.io/assets/icons/{symbol.lower()}@2x.png"

                st.subheader(f"📊 {name} ({symbol}/USDT) — ${round(price, 4)}")
